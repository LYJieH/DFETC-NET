...
class SwinTransformer():
    ...
    def call(self, input, batch_size=4,  training=None):
        x, H, W = self.patch_embed(input)
        x = self.pos_drop(x, training=training)

        skip_connection_1 = []
        skip_connection_2 = []
        # skip_1[2] = tf.reshape(skip_1[2], [1, 72*72, -1])
        # skip_1[3] = tf.reshape(skip_1[3], [1, 36*36, -1])
        # skip_1[4] = tf.reshape(skip_1[4], [1, 18*18, -1])

        skip_connection_2 = []
        x = tf.reshape(x, [batch_size, 72, 72, -1])
        # x = CoordAtt(x)
        ...
        x = tf.reshape(x, [batch_size, 36, 36, -1])
        # x = CoordAtt(x)
        x = channel_attention(x)
        # shape=(1,36,36,96)
        skip_connection_2.append(x)
        ...
        x = tf.reshape(x, [batch_size, 18, 18, -1])
        # x = CoordAtt(x)
        x = channel_attention(x)
        # shape=(1,18,18,96)
        skip_connection_2.append(x)
        ...
        x = tf.reshape(x, [batch_size, 9, 9, -1])
        skip_connection_2.append(x)
        x = tf.reshape(x, [batch_size, 9 * 9, -1])
        skip_connection_1.append(x)
        ...
        # x = Concatenate()([x, skip_connection_1[2], skip_1[4]])
        ...
        # x = Concatenate()([x, skip_connection_1[1], skip_1[3]])
        ...
        # x = Concatenate()([x, skip_connection_1[0], skip_1[2]])
...
def encoder1(inputs):
    ...
    names = ["block1_conv1", "block2_conv2", "block3_conv4", "block4_conv4", "block5_conv4"]
    for name in names:
...
def decoder1(inputs, skip_connections,skip_connection_2):
    ...
    x = Concatenate()([x, skip_connections[0],skip_connection_2[1]])
    x = conv_block(x, num_filters[0])

    x = UpSampling2D((2, 2), interpolation='bilinear')(x)
    x = Concatenate()([x, skip_connections[1],skip_connection_2[2]])
    x = conv_block(x, num_filters[1])

    x = UpSampling2D((2, 2), interpolation='bilinear')(x)
    x = Concatenate()([x, skip_connections[2],skip_connection_2[3]])
...
def build_model(shape,batch_size=4):
    inputs = Input(shape, batch_size=batch_size)
    y = SKConv2(3,G=1)(inputs)
    y,skip_2 = SwinTransformer().call(y,batch_size)
    x, skip_1 = encoder1(inputs)
    x = Concatenate()([x, y])
    x = ASPP(x,32)
    x = decoder1(x, skip_1,skip_2)
