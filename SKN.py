...
def SKConv2(M=2, r=16, L=32, G=32, name='skconv2'):
  ...
  def wrapper(inputs):
    ...
    for m in range(1, M+1):
      if G == 1:
        _x = layers.Conv2D(filters, 3, dilation_rate=m, padding='same',
                          use_bias=False, name=name+'_conv2%d'%m)(x)
      else:
        c = filters // G
        _x = layers.DepthwiseConv2D(3, dilation_rate=m, depth_multiplier=c, padding='same',
                                   use_bias=False, name=name+'_conv2%d'%m)(x)

        _x = layers.Reshape([h, w, G, c, c], name=name+'_conv2%d_reshape1'%m)(_x)
        _x = layers.Lambda(lambda x: tf.reduce_sum(_x, axis=-1),
                          output_shape=[b, h, w, G, c],
                          name=name+'_conv2%d_sum'%m)(_x)
        _x = layers.Reshape([h, w, filters],
                           name=name+'_conv2%d_reshape2'%m)(_x)


      _x = layers.BatchNormalization(name=name+'_conv2%d_bn'%m)(_x)
      _x = layers.Activation('relu', name=name+'_conv2%d_relu'%m)(_x)
    ...
    x = layers.Lambda(lambda x: tf.stack(x, axis=-1),
                      output_shape=[b, h, w, filters, M],
                      name=name+'_stack')(xs) # b, h, w, c, M
    x = Axpby2(name=name+'_axpby')([scale, x])
    return x
  return wrapper

class Axpby2(layers.Layer):
  """
  Do this:
    F = a * X + b * Y + ...
    Shape info:
  ...
  """
  def __init__(self, **kwargs):
        super(Axpby2, self).__init__(**kwargs)

  def build(self, input_shape):
        super(Axpby2, self).build(input_shape)  # Be sure to call this at the end
